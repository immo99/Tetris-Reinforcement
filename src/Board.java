...
public class Board
{
    ...
    /**
     * Gets number of empty blocks from top down
     *
     * @return number of empty blocks from top down
     */
    public int[] getEmptyBlocks() {
        int[] emptBlocks = new int[getColumns()];
        boolean[] canFill = new boolean[emptBlocks.length];
        Arrays.fill(canFill, true);


        for( int i = (totalRows - activeRows); i < totalRows; i++ ) {
            for( int j = 0; j < getColumns(); j++ ) {
                if( grid[i][j] == 0 && canFill[j] )
                    emptBlocks[j] += 1;
                else
                    canFill[j] = false;
            }
